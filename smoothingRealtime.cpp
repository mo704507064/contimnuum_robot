...
#pragma region 获取点云帧线程
	...
	std::thread th1 = thread([&]{
		while (1)
		{
			...
			for (int i = 0; i < 512 * 424; i++)
			{
				if (pixelFilterData[i]<600 && pixelFilterData[i]>500)
				{
					img.data[i] = 255;
				}
				else
			...
			//ROI 
			Mat roi = img(Rect(180, 180, 190, 70));
			...
			cv::findContours(roi, contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE, cv::Point(0, 0));
			Mat imageContours = Mat::zeros(roi.size(),CV_8UC3);
			//Mat imageContours = Mat::zeros(roi.size(), CV_8UC3);
			...
			cv::drawContours(imageContours, contours, largestContour, cv::Scalar(0,0,255), 1, 8, hierarchy);
			size_t rightContour = 0;
			if (!contours.empty())
			{
				...
				{
					if (hull[0][i].x>hull[0][rightContour].x)
						rightContour = i;
				}
				//circle(imageContours, hull[0][rightContour], 2, Scalar(0, 255, 0),1,8);
				Rect r(hull[0][rightContour].x - 6,hull[0][rightContour].y-3,6,6);
				rectangle(imageContours,r,Scalar(0,255,0),1,8);
			}
